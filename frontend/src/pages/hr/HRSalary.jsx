...
export default function HRSalary() {
  ...
  const load = async () => {
    ...
    setLoading(true);
    const params = { periodStart: startDate, periodEnd: endDate };
    if (includeRetracted) params.includeRetracted = 'true';
  ...
  // Phase 32 -- retract handler.  Confirmation copy matches the spec
  // ("Retract this salary slip?  This will remove the generated payroll
  // record and allow it to be generated again.").  Optional reason is
  // collected via prompt() so we don't need a second modal.
  const retractSlip = async (slip) => {
    const ok = window.confirm('Retract this salary slip?\nThis will remove the generated payroll record and allow it to be generated again.');
    if (!ok) return;
    const reason = window.prompt('Reason (optional). Examples: Attendance correction · Wrong payroll period · Leave correction · Generated by mistake', '');
    try {
      await api.post(`/salary/${slip._id}/retract`, { reason: reason || '' });
      toast.success('Salary slip retracted');
      setModal(null);
      load();
    } catch (err) { toast.error(errMsg(err)); }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        {/* Phase 32 -- show-retracted toggle.  Active payroll continues
            to hide retracted slips by default. */}
        <label className="flex items-center gap-2 text-xs text-slate-600 cursor-pointer mt-1">
          ...
          Show retracted slips (audit view)
        </label>
      </div>
            ...
            <tbody>
              {slips.map((s) => {
                ...
                <tr key={s._id} className={isRetracted ? 'opacity-60' : ''}>
                  ...
                  {/* Phase 32 -- status badge column */}
                  <td><StatusBadge slip={s} /></td>
                  ...
                  <td className="text-right whitespace-nowrap">
                    {!isRetracted && <button className="btn-ghost" onClick={() => openAdjust(s)}>Adjust</button>}
                    {!isRetracted && (
                      <button className="btn-ghost text-red-600" title="Retract this salary slip" onClick={() => retractSlip(s)}>
                        Retract
                      </button>
                    )}
      ...
      {modal && (
        <AdjustModal modal={modal} setModal={setModal} onSave={updateSlip} onRetract={retractSlip} />
      )}
    </div>
  );
}
 ...
 */
function AdjustModal({ modal, setModal, onSave, onRetract }) {
  ...
  return (
    <Modal
      ...
      footer={<>
        {/* Phase 32: Retract sits on the left so it's visually
            distinct from the Save action. */}
        {onRetract && modal.status !== 'retracted' && (
          <button className="btn-ghost text-red-600 mr-auto" onClick={() => onRetract(modal)}>
            Retract Salary
          </button>
        )}
...
/* =====================================================================
 * Phase 32 — Status Badge
 *
 * Renders Generated (green) / Retracted (red) / Paid (blue) per the
 * spec.  Retracted badges also show who pulled the slip back and when,
 * so HR can audit the action at a glance.
 * ===================================================================== */
function StatusBadge({ slip }) {
  const status = slip?.status || 'active';
  if (status === 'retracted') {
    const when = slip.retractedAt ? new Date(slip.retractedAt).toLocaleString() : '';
    const who  = slip.retractedBy?.name || '';
    const title = `Retracted${who ? ' by ' + who : ''}${when ? ' on ' + when : ''}${slip.retractionReason ? ' · Reason: ' + slip.retractionReason : ''}`;
    return <span className="badge bg-red-50 text-red-700 border border-red-200 dark:bg-red-500/20 dark:text-red-300" title={title}>Retracted</span>;
  }
  if (status === 'paid') {
