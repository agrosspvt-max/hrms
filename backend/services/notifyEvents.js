...
/**
 * Phase-1 dedupe writer.  Every "one-shot" business event (penalty
 * applied, leave decided, salary generated, etc.) should route
 * through this helper INSTEAD of `_emit`.  The (recipient, eventKey,
 * variant) partial unique index on Notification collapses any
 * duplicate at the DB level, so a cron restart, React double-mount,
 * SSE-driven refetch, or two concurrent writers all produce exactly
 * ONE row.  Returns `{ created, doc }`: callers should only emit a
 * realtime frame when `created === true`.
 */
const _upsertOne = async ({ recipient, eventKey, variant = 'default', sourceRef, payload }) => {
  if (!recipient || !eventKey) return { created: false, doc: null };
  try {
    const now = new Date();
    const res = await Notification.findOneAndUpdate(
      { recipient, eventKey, variant },
      {
        $setOnInsert: {
          recipient,
          eventKey,
      ...
      { new: false, upsert: true, rawResult: true, setDefaultsOnInsert: true },
    );
    // Mongoose returns `lastErrorObject.updatedExisting === false` on insert.
    const created = !!(res?.lastErrorObject && res.lastErrorObject.updatedExisting === false);
    return { created, doc: res?.value || null };
  } catch (err) {
    // Race: two writers hit the same key at the same instant.  The
    // partial unique index rejects the loser with duplicate key --
    // treat as "already existed, we did not create it".
    if (err && err.code === 11000) return { created: false, doc: null };
    console.error('[notify] upsert failed:', err.message);
...
const notifySalarySlipGenerated = async ({ employeeId, slip, generatedBy }) => {
  try {
    ...
    // Phase-1 dedupe: one slip -> one notification, always.
    const { created } = await _upsertOne({
      recipient: employeeId,
      eventKey: `salary.slip.generated:${String(slip?._id || period)}`,
      variant: 'default',
      sourceRef: slip?._id ? { module: 'salary_slip', id: slip._id } : undefined,
      payload: {
        type: 'general',
    ...
    if (created) rt.publish(employeeId, 'notification:new', { priority: 'normal' });
    ...
    return { count: created ? 1 : 0, deduped: !created };
  } catch (e) {
 ...
 */
const notifyPenalty = async ({ employeeId, penalty, mode /* 'probable' | 'active' */, event }) => {
  try {
    ...
    // Phase-1: dedupe by (recipient, eventKey, variant).  One PENALTY
    // + one EVENT (applied / probable / waived / resolved / deducted /
    // restored) produces at most ONE notification, regardless of how
    // many times a caller invokes this helper.  Callers can supply an
    // explicit `event` string to differentiate state transitions on
    // the SAME penalty; when omitted we fall back to the mode.
    const eventTag = String(event || (isProbable ? 'probable' : 'applied'));
    const { created } = await _upsertOne({
      recipient: employeeId,
      eventKey: `penalty.${eventTag}:${String(penalty._id)}`,
      variant: 'default',
      sourceRef: { module: 'penalty', id: penalty._id },
      payload: { type: 'general', title, message, priority },
    });
    // Realtime frames are the ONLY thing gated on `created` -- we don't
    // spam the client with "notification:new" for a row that already
    // existed.  penalty:changed still fires so the Fines & Penalties
    // page refreshes on every actual state change; callers of this
    // helper only invoke it on real transitions.
    if (created) rt.publish(employeeId, 'notification:new', { priority });
...
module.exports = {
  ...
  notifyPenalty,
  // Phase-2 cleanup: sanctioned dedupe writer exposed for new
  // subscribers (meeting notifications, future one-shot events).
  // Every caller MUST provide a stable `eventKey`.
  _upsertOne,
};
